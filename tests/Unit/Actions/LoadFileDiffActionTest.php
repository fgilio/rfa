...
test('returns full DTO array for modified file', function () {
    ...
    $action = new LoadFileDiffAction(new GitDiffService(new GitProcessService, new IgnoreService), new DiffParser, new SyntaxHighlightService, new MarkdownTableAlignerService, new CsvAlignerService, new MarkdownRegionService, new DiffSplitPairerService);
...
test('returns error field when git command fails', function () {
    ...
    $action = new LoadFileDiffAction($gitService, new DiffParser, new SyntaxHighlightService, new MarkdownTableAlignerService, new CsvAlignerService, new MarkdownRegionService, new DiffSplitPairerService);
    $result = $action->handle($this->tmpDir, 'hello.txt');
